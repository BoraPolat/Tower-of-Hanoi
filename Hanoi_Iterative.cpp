...
void hanoi() {
    ...
    for (int i = 1; i <= total_moves; i++) {
        if (n % 2 == 1) {
            if      (i % 3 == 1) { if (get_top(A) < get_top(C)) pop_and_push(A, C); else pop_and_push(C, A); }
            else if (i % 3 == 2) { if (get_top(A) < get_top(B)) pop_and_push(A, B); else pop_and_push(B, A); }
            else                  { if (get_top(B) < get_top(C)) pop_and_push(B, C); else pop_and_push(C, B); }
        } else {
            if      (i % 3 == 1) { if (get_top(A) < get_top(B)) pop_and_push(A, B); else pop_and_push(B, A); }
            else if (i % 3 == 2) { if (get_top(A) < get_top(C)) pop_and_push(A, C); else pop_and_push(C, A); }
            else                  { if (get_top(B) < get_top(C)) pop_and_push(B, C); else pop_and_push(C, B); }
        }
...
int main() {
    ...
    log_file.open("hanoi_iterative_output.txt", ios::trunc);
